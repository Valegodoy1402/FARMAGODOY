...
<div class="ventas-index">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
        <div>
            ...
            <p class="text-muted mb-0 small">Historial de facturación y transacciones monetarias de FarmaciaGodoy</p>
        </div>
        <div>
            <?= Html::a('<span class="me-1">+</span> Nueva Venta', ['create'], [
                ...
                'columns' => [
                    ...
                    [
                        ...
                        'value' => function ($model) {
                            $iconColor = ($model->venta_id % 2 === 0) ? '#e0f2fe' : '#e0f2fe';
                            $textColor = ($model->venta_id % 2 === 0) ? '#029bf1' : '#0c385a';
                            return '<div class="d-flex align-items-center gap-2">
                                        <span class="badge rounded p-2" style="background-color: '.$iconColor.'; color: '.$textColor.'; font-size: 0.85rem;">🛍️</span>
                                        <span class="fw-semibold text-dark" style="font-size: 0.92rem;">#' . Html::encode($model->venta_id) . '</span>
                                    </div>';
                        }
                    ],
                    [
                        ...
                        'headerOptions' => ['class' => 'text-uppercase text-muted fw-bold', 'style' => 'font-size: 0.75rem; letter-spacing: 0.5px;'],
                        'contentOptions' => ['style' => 'font-size: 0.9rem; color: #4a5568;'],
                    ],
                    [
                    ...
                    [
                        ...
                        'value' => function ($model) {
                            return $model->fecha_venta ? date('d/m/Y H:i', strtotime($model->fecha_venta)) : '-';
                        }
                    ],
                    [
                        'attribute' => 'cantidad',
                        ...
                        'format' => 'raw',
                        'value' => function ($model) {
                            return '<span class="badge rounded-1 px-2 py-1 bg-opacity-10 bg-secondary text-dark" style="font-size: 0.8rem; font-weight: 600;">' . Html::encode($model->cantidad) . ' u.</span>';
                        }
                    ],
                    [
                        ...
                        'urlCreator' => function ($action, Ventas $model, $key, $index, $column) {
                            return Url::toRoute([$action, 'venta_id' => $model->venta_id]);
                        },
                        ...
                        'buttons' => [
                            ...
                            'delete' => function ($url, $model, $key) {
                                return Html::a('🗑️', $url, [
                                    ...
                                    'data' => [
                                        'confirm' => '¿Estás seguro de que deseas eliminar esta transacción de venta?',
                                        'method' => 'post',
                                    ],
                                ]);
                            },
